import { createRootRouteWithContext, createRoute, createRouter, createBrowserHistory, redirect, useRouteContext, useRouter } from "@tanstack/react-router";
import type { RouterHistory } from "@tanstack/react-router";
import { Outlet } from "@tanstack/react-router";
import { AppShell, Callback, ComingSoon, Landing, LeagueHome, LeaguePage, SetPassword, SignIn, StatePage, SystemAdmin } from "./App";
import type { AuthController } from "./lib/auth";
import type { ApiRuntime } from "./lib/api";
import { sessionQueryOptions } from "./lib/session";
import { rememberDestination, safeDestination } from "./lib/destination";
...
const signInRoute = createRoute({ getParentRoute: () => rootRoute, path: "/sign-in", validateSearch: (search) => ({ next: safeDestination(search.next), reason: search.reason === "unauthorized" ? "unauthorized" : undefined }), component: SignIn });
...
const systemAdminRoute = createRoute({ getParentRoute: () => appRoute, path: "/admin", beforeLoad: async ({ context }) => { const session = await ensureSession(context); if (!session.account.is_system_owner) throw redirect({ to: "/app" }); }, component: SystemAdmin });
...
const adminRoute = createRoute({ getParentRoute: () => leagueRoute, path: "/admin", beforeLoad: async ({ context, params }) => { const session = await ensureSession(context); const league = session.leagues.find((item) => item.id === params.leagueId); if (!league?.is_commissioner || league.read_only) throw redirect({ to: "/app" }); }, component: ComingSoon });
...
export function createAppRouter(context: RouterContext, history?: RouterHistory) { return createRouter({ routeTree, context, defaultPreload: "intent", history: history ?? createBrowserHistory() }); }
