...
import { sessionQueryOptions, type SessionResponse } from "./lib/session";
...
const systemAdminRoute = createRoute({ getParentRoute: () => appRoute, path: "/admin", beforeLoad: ({ context }) => { const session = context.runtime.queryClient.getQueryData<SessionResponse>(["session"]); if (!session?.account.is_system_owner) throw redirect({ to: "/app" }); }, component: ComingSoon });
const leagueRoute = createRoute({ getParentRoute: () => appRoute, path: "/leagues/$leagueId", beforeLoad: ({ context, params }) => { const session = context.runtime.queryClient.getQueryData<SessionResponse>(["session"]); if (!session?.leagues.some((league) => league.id === params.leagueId)) throw redirect({ to: "/app" }); }, component: LeaguePage });
...
const adminRoute = createRoute({ getParentRoute: () => leagueRoute, path: "/admin", beforeLoad: ({ context, params }) => { const session = context.runtime.queryClient.getQueryData<SessionResponse>(["session"]); if (!session?.leagues.find((league) => league.id === params.leagueId)?.is_commissioner) throw redirect({ to: "/app" }); }, component: ComingSoon });
